...
1. ucore的内存布局中，页表、用户栈、内核栈在逻辑地址空间中的位置？
   ...
   答：页表在 0xFAC00000;用户栈顶在0xB0000000。
...
1. 在do_execve中的的当前进程如何清空地址空间内容的？在什么时候开始使用新加载进程的地址空间？
   ...
   答：清空进程地址空间是在initproc所在进程地址空间;CR3设置成新建好的页表地址后，开始使用新的地址空间。
...
1. 第一个内核线程和第一个用户进程的创建有什么不同？
   ...
   答：相应线程的内核栈创建时，多了SS和ESP的设置；用户进程需要创建用户地址空间，并把用户代码复制到用户地址空间。
...
1. 为什么新进程的内核堆栈可以先于进程地址空间复制进行创建？
   ...
   答：因为内核栈在进程的内核地址空间，而各进程的内核地址空间是共享的；

2. 进程复制的代码在哪？复制了哪些内容？
3. 进程复制过程中有哪些修改？为什么要修改？

   答：修改了内核栈、页表、trapframe、context、PCB字段修改。
