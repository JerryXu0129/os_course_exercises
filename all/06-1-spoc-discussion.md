...
1. 处理机调度的功能是什么？
   ...
   答：选进程，从就绪队列中挑选下一个占用CPU运行的进程;选CPU，从多个可用CPU中挑选就绪进程可使用的CPU资源。

2. 在什么时候可以进行处理机调度？

   答：在当前进程主动放弃或被抢先时进行处理机调度。

3. 当操作系统的处理机调度导致线程切换时，暂停线程的当前指令指针可能在什么位置？用户态代码或内核代码？给出理由。

   答：暂停线程的当前指令指针可能在用户态代码，因为系统调用返回时可能出现进程切换。
...
2. 处理机调度的目标是什么？
   ...
   答：CPU利用率（CPU使用率、吞吐率、周转时间、等待时间）、用户感受（周转时间、等待时间、响应时间、响应时间的方差）、公平性（CPU时间分配公平性）。
...
1. 尝试描述下列处理机调度算法的工作原理和算法优缺点。
   ...
   答：先来先服务算法按照进入就绪队列的顺序进行调度，优点是实现简单，缺点是等待时间波动性大，CPU利用率低;短进程优先算法按照占用CPU的预测时间长短进行调度，优点是等待时间最短，缺点是会出现饥饿现象;最高响应比优先算法是短进程优先算法的改进，将等待时间作为因素之一进行调度，在兼顾了等待时间较短的同时避免了饥饿现象。
...
1. 尝试描述下列处理机调度算法的工作原理和算法优缺点。
   ...
   答：时间片轮转算法是将每个进程限制一个相同的时间片，时间片结束则依据先来先服务算法进行调度，优点是对所有进程公平，缺点是无法根据进程的特点灵活分配时间;多级反馈队列算法是将多个进程依据其特点分配在不同的就绪队列中，优点是提高了CPU的效率，缺点是实现复杂;公平共享调度算法是综合了上述算法的算法。
...
5. 为什么要引入调度框架？定义调度算法接口需要考虑哪些因素？
   ...
   答：为了分离调度操作和调度策略，以支持多种调度算法；定义调度算法接口的考虑因素有调度算法需要的调度操作类型、调度算法在各调度操作中的体现方式。
...
3. 尝试描述下列处理机调度算法的工作原理和算法优缺点。
   ...
   答：速率单调调度算法为静态分配调度算法，在进程执行前先按进程执行的周期进行优先级排序与就绪队列规划，优点是开销小;最早截止时间优先算法为动态分配调度算法，在进程执行过程中按照截止时间的先后顺序进行优先级排序与就绪队列规划，缺点是实现复杂。
...
1. 什么是优先级反置现象？
   ...
   阿答：操作系统中出现高优先级进程长时间等待低优先级进程所占用资源的现象

2. 什么是优先级继承(Priority Inheritance)和优先级天花板协议(priority ceiling protocol)？它们的区别是什么？

   答：优先级继承是指占用资源的低优先级进程继承申请资源的高优先级进程的优先级;优先级天花板协议是指占用资源进程的优先级和所有可能申请该资源的进程的最高优先级相同。区别是提升占用资源的低优先级进程的优先级的时机不同
